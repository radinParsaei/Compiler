...
//			case ">=":
	...
	@ParserEvent(map = "exp : OP_SQ_BRACKET( SEP)?( exp( SEP)? COMMA( SEP)?)*( exp)?( SEP)? CL_SQ_BRACKET", priority = 21)
	...
	@ParserEvent(map = "exp : ! exp", priority = 25)
	public Object not(Parser parser) {
		setCounter(8);
		return new SyntaxTree.Not((ValueBase) parser.getTokens().get(1).getObject());
	}

	@ParserEvent(map = "program : ((VAR )?set exp|vard( exp)?|exp DOT set exp) SEP", priority = 26)
