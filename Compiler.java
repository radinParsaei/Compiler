...
//			case ">=":
	...
	@ParserEvent(map = "exp : OP_SQ_BRACKET( SEP)?( exp( SEP)? COMMA( SEP)?)*( exp)?( SEP)? CL_SQ_BRACKET", priority = 22)
	...
	@ParserEvent(map = "exp : OP_PAREN exp CL_PAREN", priority = 23)
	public Object parentheses(Parser parser) {
		setCounter(8);
		return parser.getTokens().get(1).getObject();
	}

	@ParserEvent(map = "program : inc exp( SEP)?", priority = 24)
	...
	@ParserEvent(map = "exp : ! exp", priority = 26)
	public Object not(Parser parser) {
		setCounter(8);
		return new SyntaxTree.Not((ValueBase) parser.getTokens().get(1).getObject());
	}

	@ParserEvent(map = "program : ((VAR )?set exp|vard( exp)?|exp DOT set exp) SEP", priority = 27)
	...
	@ParserEvent(map = "program : RET (exp )?SEP", priority = 28)
	...
	@ParserEvent(map = "program : BR SEP", priority = 29)
	public Object _break(Parser parser) {
		return new SyntaxTree.Break();
	}

	@ParserEvent(map = "program : CON SEP", priority = 30)
	public Object _continue(Parser parser) {
		return new SyntaxTree.Continue();
	}

	@ParserEvent(map = "program : program (SEP )?(program ?)+", priority = 31)
	...
	@ParserEvent(map = "program : WH exp (SEP )?OP_BRACKET (SEP )?(program )?CL_BRACKET SEP", priority = 32)
	...
	@ParserEvent(map = "program : FOR (OP_PAREN )?program exp SEP program (CL_PAREN )?(SEP )?OP_BRACKET (SEP )?(program )?CL_BRACKET SEP", priority = 33)
	...
	@ParserEvent(map = "program : IF exp (SEP )?OP_BRACKET (SEP )?(program )?CL_BRACKET( ELSE IF exp (SEP )?OP_BRACKET (SEP )?(program )?CL_BRACKET)*( ELSE( SEP)? OP_BRACKET( SEP)? (program )?CL_BRACKET)? SEP", priority = 34)
	...
	@ParserEvent(map = "program : fn CL_PAREN (SEP )?OP_BRACKET (SEP )?(program )?CL_BRACKET SEP", priority = 35)
	...
	@ParserEvent(map = "exp : (OP_PAREN CL_PAREN ARROW|lambda) OP_BRACKET (SEP )?(program )?CL_BRACKET", priority = 36)
	...
	@ParserEvent(map = "program : class (SEP )?OP_BRACKET (SEP )?program CL_BRACKET", priority = 37)
	...
	@ParserEvent(map = "program : exp SEP", priority = 38)
