public class Web {
    public static void parse(Parser parser, Compiler compiler) {
        ...
        for (compiler.setCounter(0); compiler.getCounter() < 31; compiler.increaseCounter()) {
            ...
            if (compiler.getCounter() == 11) parser.on("ID (OP2|OP1|OP3|E) SET exp SEP", "program", compiler::increaseAndOthers);
            ...
            if (compiler.getCounter() == 21) parser.on("RET (exp )?SEP", "program", compiler::_return);
            if (compiler.getCounter() == 22) parser.on("fnc exp", "fnc", compiler::functionCall2);
            if (compiler.getCounter() == 23) parser.on("fnc COMMA exp", "fnc", compiler::functionCall3);
            if (compiler.getCounter() == 24) parser.on("program (SEP )?(program ?)+", "program", compiler::programs);
            ...
            if (compiler.getCounter() == 28) parser.on("(OP_PAREN CL_PAREN ARROW|lambda) OP_BRACKET (SEP )?program CL_BRACKET", "exp", compiler::lambda);
            if (compiler.getCounter() == 29) parser.on("fnc CL_PAREN", "exp", compiler::functionCall4);
            if (compiler.getCounter() == 30) parser.on("exp SEP", "program", compiler::executeValue);
        }
    }
}
