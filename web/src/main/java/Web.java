...
public class Web {
    public static void parse(Parser parser, Compiler compiler) {
        ...
        for (compiler.setCounter(0); compiler.getCounter() < 42; compiler.increaseCounter()) {
            ...
            if (compiler.getCounter() == 17) parser.on("exp E exp(?! DOT)", "exp", compiler::exponentiation);
            if (compiler.getCounter() == 18) parser.on("exp OP2 exp(?! DOT)", "exp", compiler::additionAndSubtraction);
            if (compiler.getCounter() == 19) parser.on("exp COMP exp(?! DOT)", "exp", compiler::comparison);
            if (compiler.getCounter() == 20) parser.on("exp OP3 exp(?! DOT)", "exp", compiler::bitwiseAnd);
            if (compiler.getCounter() == 21) parser.on("new OP_PAREN ((exp COMMA )*exp )?CL_PAREN", "exp", compiler::createInstance1);
            if (compiler.getCounter() == 22) parser.on("exp (OP_SQ_BRACKET( SEP)? exp( SEP)? CL_SQ_BRACKET( SEP)? )+SET( SEP)? exp SEP", "program", compiler::setInArray);
            ...
            if (compiler.getCounter() == 25) parser.on("OP_SQ_BRACKET( SEP)?( exp( SEP)? COMMA( SEP)?)*( exp)?( SEP)? CL_SQ_BRACKET", "exp", compiler::array);
            if (compiler.getCounter() == 26) parser.on("OP_PAREN exp CL_PAREN", "exp", compiler::parentheses);
            if (compiler.getCounter() == 27) parser.on("exp ! OP_PAREN (exp )?(COMMA exp )*CL_PAREN", "exp", compiler::callFunctionFromPointer);
            if (compiler.getCounter() == 28) parser.on("inc exp( SEP)?", "program", compiler::increaseAndOthers1);
            ...
            if (compiler.getCounter() == 39) parser.on("(OP_PAREN CL_PAREN ARROW|lambda) OP_BRACKET (SEP )?(program )?CL_BRACKET", "exp", compiler::lambda);
            if (compiler.getCounter() == 40) parser.on("class (SEP )?OP_BRACKET (SEP )?(program )?CL_BRACKET", "program", compiler::createClass1);
            if (compiler.getCounter() == 41) parser.on("exp SEP", "program", compiler::executeValue);
        }
    }
}
