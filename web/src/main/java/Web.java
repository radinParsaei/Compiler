...
public class Web {
    public static void parse(Parser parser, Compiler compiler) {
        ...
        for (compiler.setCounter(0); compiler.getCounter() < 41; compiler.increaseCounter()) {
            ...
            if (compiler.getCounter() == 15) parser.on("exp OP1 exp(?! DOT)", "exp", compiler::multiplyAndDivideAndRemainder);
            if (compiler.getCounter() == 16) parser.on("exp E exp(?! DOT)", "exp", compiler::exponentiation);
            if (compiler.getCounter() == 17) parser.on("exp OP2 exp(?! DOT)", "exp", compiler::additionAndSubtraction);
            if (compiler.getCounter() == 18) parser.on("exp COMP exp(?! DOT)", "exp", compiler::comparison);
            if (compiler.getCounter() == 19) parser.on("exp OP3 exp(?! DOT)", "exp", compiler::bitwiseAnd);
            ...
            if (compiler.getCounter() == 26) parser.on("exp ! OP_PAREN (exp )?(COMMA exp )*CL_PAREN", "exp", compiler::callFunctionFromPointer);
            if (compiler.getCounter() == 27) parser.on("inc exp( SEP)?", "program", compiler::increaseAndOthers1);
            ...
            if (compiler.getCounter() == 32) parser.on("CON SEP", "program", compiler::_continue);
            if (compiler.getCounter() == 33) parser.on("program (SEP )?(program ?)+", "program", compiler::programs);
