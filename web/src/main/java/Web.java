...
public class Web {
    public static void parse(Parser parser, Compiler compiler) {
        ...
        for (compiler.setCounter(0); compiler.getCounter() < 37; compiler.increaseCounter()) {
            ...
            if (compiler.getCounter() == 21) parser.on("OP_PAREN exp CL_PAREN", "exp", compiler::parentheses);
            if (compiler.getCounter() == 22) parser.on("inc exp( SEP)?", "program", compiler::increaseAndOthers1);
            ...
            if (compiler.getCounter() == 28) parser.on("CON SEP", "program", compiler::_continue);
            if (compiler.getCounter() == 29) parser.on("program (SEP )?(program ?)+", "program", compiler::programs);
            ...
            if (compiler.getCounter() == 34) parser.on("(OP_PAREN CL_PAREN ARROW|lambda) OP_BRACKET (SEP )?(program )?CL_BRACKET", "exp", compiler::lambda);
            if (compiler.getCounter() == 35) parser.on("class (SEP )?OP_BRACKET (SEP )?program CL_BRACKET", "program", compiler::createClass1);
            if (compiler.getCounter() == 36) parser.on("exp SEP", "program", compiler::executeValue);
        }
    }
}
